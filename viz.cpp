...
void Viz::toPly(string filename, vector<GLfloat> &vertices, vector<GLuint> &indices, vector<GLfloat> &normal, vector<GLfloat> &colors) {
  FILE *fo = fopen(filename.c_str(), "w");
  fprintf(fo, "ply\nformat ascii 1.0\nelement vertex %d\nproperty float x\nproperty float y\nproperty float z\nproperty uchar red\nproperty uchar green\nproperty uchar blue\nelement face %d\nproperty list uchar int vertex_indices\nend_header\n", vertices.size() / 3, indices.size() / 3);
  for (int i = 0; i < vertices.size(); i+= 3) {
    fprintf(fo, "%lf %lf %lf %f %f %f\n", vertices[i], vertices[i+1], vertices[i+2], colors[i], colors[i+1], colors[i+2]);
  }
  for (int i = 0; i < indices.size(); i+= 3) {
...
void Viz::setIdleCallback(void (*func)()) {
  idleFunc = func;
}
...
void Viz::updateProjection() {
  ...
  float sh = 10;
  float minDst = mult - sh < 0.1 ? 0.1 : mult - sh;
  float maxDst = mult + sh;
...
void Viz::display() {
  ...
  if (drawMesh) {
    ...
    if (useColor) {
      ...
      glColorPointer(3, GL_FLOAT, 0, &Viz::colors[0]);
      glEnable(GL_COLOR_MATERIAL);
    } else {
      glDisable(GL_COLOR_MATERIAL);
    }
...
void Viz::loadPly(string filename, vector<GLfloat> &vertices, vector<GLuint> &indices, vector<GLfloat> &colors) {
  vertices.clear();
  colors.clear();
  indices.clear();
  _vertices = &vertices;
...
double Viz::normalizeSurface(vector<GLfloat> &v) {
  float c[3][2];
  float r[3];
  float mr;
  ...
  return Viz::normScale;
}

double Viz::normalizeSurface() {
  return Viz::normalizeSurface(Viz::vertices);
}
