...
elif FLAGS.dataset in ["acup5", "acup7"]:
...
def setGlobalVariables():
  ...
  if dmin <0 and dmax<0:
     ...
     ccz += 0.1
     ccx -= 0.05
     #dis=np.linalg.norm(np.transpose(ref_c[FLAGS.index]) - np.array([ccx,ccy,ccz]),2)
     #cop = (dx+dy+dz)/3
     #dmin = dis-cop
     #dmax = dis+cop
...
def train():
    ...
    int_mpi1 = np.random.uniform(-5,3,[num_mpi, h, w, 3]).astype(np.float32)
    int_mpi2 = np.random.uniform(-5,-3,[num_mpi, h, w, 1]).astype(np.float32)
    int_mpi = np.concatenate([int_mpi1,int_mpi2],-1)

    backup = tf.get_variable("Net_backup", initializer=latent, trainable=True)
    ...
    bg = tf.get_variable("Net_backupbg", initializer=np.array([1,1,1],dtype=np.float32), trainable=True)
    bg = tf.sigmoid(bg)
    ...
    for i in range(num_):
        ...
        with tf.compat.v1.variable_scope("loss%d"%((FLAGS.index+i)%mpi_max)):
            tva = tf.constant(0.01)
            tvc = tf.constant(0.001)
            mpiColor = mpi[:, :, :, :3]
            mpiAlpha = mpi[:, :, :, 3:4]
    ...
    for i_mpi in range(num_):
        ...
        for i in range(FLAGS.epoch + 3):
            ...
            if (i_mpi==0 and i<250):
                _,los = sess.run([train_ops[i_mpi],losses[i_mpi]],feed_dict={lod_in:i,rot:feed["r"][0],tra:feed["t"][0],real_img:feed["img"][0]})
            else:
                _,los = sess.run([train_ops[i_mpi],losses[i_mpi]],feed_dict={lod_in:i,tva:0.001,rot:feed["r"][0],tra:feed["t"][0],real_img:feed["img"][0]})

            if i%100==0:
                print(FLAGS.index,i_mpi,i, "loss = " ,los)
            if i%20 == 0:
               if(FLAGS.index>mpi_max): sess.run(loadup)
               if(FLAGS.epoch - i < 20):
                 summ = sess.run(summaries[i_mpi],feed_dict={lod_in:i,rot:feed0["r"][0],tra:feed0["t"][0],real_img:feed0["img"][0]})
               else:
                 summ = sess.run(summaries[i_mpi],feed_dict={lod_in:i,rot:feed["r"][0],tra:feed["t"][0],real_img:feed["img"][0]})
