...
def gen_mpi(input,is_train):
  with tf.variable_scope('gen',reuse=tf.AUTO_REUSE) :
    #scope.reuse_variables()=tf.AUTO_REUSE
    with slim.arg_scope(
        ...
        for i in range(3):
          next = next + slim.conv2d(next,75,[3,3])
        ...
        for i, lay in reversed(list(enumerate(layers))):
            ...
            next = slim.conv2d(next,64,[3,3])
        
        next = slim.conv2d(next,3,[3,3])
        nois = tf.compat.v1.get_variable("alpha0",shape=[num_mpi,nh,nw,1], trainable=True)
        next = tf.concat([next,nois],-1)
        next = slim.conv2d(next,4,[3,3],activation_fn=tf.sigmoid)
    return next
...
def gen_alpha(input,is_train,reuse=False):
    with tf.compat.v1.variable_scope('alpha') as scope:
        if reuse: scope.reuse_variables()
        ...
        with slim.arg_scope(
            ...
            for i in range(3):
              next = next + slim.conv2d(next,64,[3,3])
            ...
            next = slim.conv2d(next,20,[3,3])
            next = slim.conv2d(next,sub_sam,[3,3],activation_fn=tf.sigmoid)
    return next
...
def predict():
    ...
    if True:
      ...
      for ii in range(1):
        ...
        print(ref_rt.shape)
        ret, sublay = sess.run([mpi,sss],feed_dict={features['r']:[ref_r],features['t']:[ref_t]})
        sublayers = []
        mpis = []
        sublayers_combined = []
